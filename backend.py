...
def _try_split_image_translation(
    ...
    def get_half(messages, label):
        return _get_relay_result(
            ...
            temperature,
            max_tokens,
            # Tried dropping this to 1 (2026-09), but real usage showed
            # documents that used to succeed on the 2nd attempt started
            # failing the whole cascade outright (main + both halves +
            # fallback model all failing with the same "repetition
            # detected" reason) — the 2nd attempt DOES sometimes recover
            # where the 1st looped. Reverted: reliability over speed.
            max_repetition_attempts=2,
            label=label,
        )
...
ANALYSIS_PROMPT = (
    ...
    "trans_modern alanı için: translit alanındaki Osmanlıca metni "
    "doğrudan kaynak alarak, akıcı ve doğal modern Türkçeye çevir. Bu bir "
    "ÇEVİRİDİR — özet, açıklama veya yorum DEĞİLDİR. "

    "Kesin kurallar: "
    "1. translit'teki HER cümleye karşılık gelen bir modern Türkçe cümle "
    "üret, sırasıyla ve hiçbir cümleyi atlamadan veya birleştirmeden. "
    "2. Eğer metin soru-cevap formatındaysa (sual/cevap gibi), "
    "trans_modern da AYNI soru-cevap formatını korumalıdır. Üçüncü şahıs "
    "anlatımına ('...olduğu söylenir', '...belirtilir', '...anlatılır') "
    "ASLA çevirme. "
    "3. Anlamı asla değiştirme, daraltma, yorumlama veya atlama — "
    "çevirinin görevi sadece dili güncellemektir, içeriği yeniden "
    "yorumlamak değil. "
    "4. Cümle yapısını doğal modern Türkçeye uyarla (gerekirse uzun "
    "izafet zincirlerini kısa cümlelere böl), ama bu, bilgi eklemek veya "
    "çıkarmak anlamına gelmez. "

    "Kelime seçimi: Osmanlıca/Arapça/Farsça kökenli olup günümüzde "
    "günlük konuşmada artık nadiren kullanılan, resmi/edebi duran "
    "kelimeler yerine, üniversite öğrencisinin veya müzede bu metni "
    "okuyan sıradan bir ziyaretçinin her gün kullandığı kelimeleri "
    "tercih et. "

    "Örnekler (kelime -> günlük karşılığı): tasavvur -> düşünce/hayal "
    "etme; tecelli -> ortaya çıkma/gerçekleşme; tebarüz etmek -> öne "
    "çıkmak; muhtelif -> çeşitli/farklı; nezdinde -> gözünde/yanında; "
    "hasebiyle -> yüzünden/nedeniyle. "

    "Eğer bir terimin (özellikle dinî, hukukî veya tarihî bir kavramın) "
    "günlük dilde tam karşılığı yoksa, terimi olduğu gibi kullan; "
    "gerekirse parantez içinde tek kelimelik kısa açıklama ekle. Örnek: "
    "'vakıf (hayır kurumu)'. "

    "ÖRNEK (cümle cümle çeviri, format koruma): Osmanlıca cümle: "
    "'سؤال: اول وزیر كیمدر؟ جواب: روم محمد پاشا' — Doğru trans_modern: "
    "'Soru: O vezir kimdir? Cevap: Rum Mehmed Paşa.' — YANLIŞ (yapma): "
    "'İlk vezirin Rum Mehmed Paşa olduğu söylenir.' "

    "translit/ocr alanlarında **çift yıldızla** işaretlenmiş (tahmin "
    "edilmiş) kısımların buradaki karşılığını da aynı şekilde **çift "
    "yıldız** içine alarak işaretle — bu işaretleme kelime alternatifi "
    "özelliğinin çalışması için gereklidir. "
...
def translate_endpoint():
    ...
    try:
        ...
        main_result = _get_relay_result(
            ...
            # get_half(): dropping this to 1 caused documents that used
            # to succeed on the 2nd attempt to fail the whole cascade
            # outright. Reverted to 2 — reliability over speed.
            max_repetition_attempts=2,
            label="main",
        )
        ...
        # reason == "unusable": both attempts either looped, returned
        # empty/invalid JSON, or were missing ocr/trans, after already
        # retrying once internally. Instead of failing right away, try
        # splitting the image into top/bottom halves — a difficult
        # full-page image often succeeds once each half is a simpler,
        # smaller request.
